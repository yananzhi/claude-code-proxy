...
// ════════════════════════════════════════════════════════════
// D4 PTY 终端生命周期（mock pty，keyed by terminalId）
// ════════════════════════════════════════════════════════════
// 新模型：start(terminalId, {cwd, binaryPath, env, configDir, workspaceId, configId, startedConfigName, kind})
//   永不 reuse，每次 spawn 新 PTY。支持同一 workspace/config 多终端。

function makeStartParams(overrides = {}) {
    return {
        cwd: newTmpDir('params'),
        binaryPath: '/fake/claude',
        env: {},
        configDir: newTmpDir('cfgdir'),
        workspaceId: 'ws_x',
        configId: 'cfg_x',
        startedConfigName: 'test-cfg',
        kind: 'normal',
        ...overrides,
    };
}

test('D4a: 启动终端 → PTY spawn + 入 Map + 返回 terminalId/pid', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    const result = await mgr.start('t1', makeStartParams());
    assert.equal(result.terminalId, 't1');
    assert.ok(result.pid > 0);
    assert.ok(mgr.status('t1'));
    await mgr.stop('t1');
});

test('D4b: 同 workspace 开两个终端 → 两个独立 PTY（不 reuse）', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    const r1 = await mgr.start('t1', makeStartParams({ workspaceId: 'ws1' }));
    const r2 = await mgr.start('t2', makeStartParams({ workspaceId: 'ws1' }));
    assert.notEqual(r1.terminalId, r2.terminalId);
    assert.notEqual(r1.pid, r2.pid);
    assert.equal(mockPty._handles.length, 2, '应 spawn 两个独立 PTY');
    assert.ok(mgr.status('t1') && mgr.status('t2'));
    await mgr.stop('t1');
    await mgr.stop('t2');
});

test('D4c: 停止终端 → kill + 出 Map', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t3', makeStartParams());
    assert.equal(await mgr.stop('t3'), true);
    assert.equal(mgr.status('t3'), null);
});

test('D4d: 停止不存在的终端 → false', async () => {
...
test('D4e: PTY 自然退出 → onExit 清 Map', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t4', makeStartParams());
...
test('D4f: listByWorkspace → 返回该 workspace 的活终端', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t1', makeStartParams({ workspaceId: 'wsA', configId: 'c1' }));
    await mgr.start('t2', makeStartParams({ workspaceId: 'wsA', configId: 'c2' }));
    await mgr.start('t3', makeStartParams({ workspaceId: 'wsB', configId: 'c3' }));
    const aTerms = mgr.listByWorkspace('wsA');
    assert.equal(aTerms.length, 2);
    assert.ok(aTerms.every(t => t.workspaceId === 'wsA'));
    const bTerms = mgr.listByWorkspace('wsB');
    assert.equal(bTerms.length, 1);
    await mgr.stopAll();
});
...
test('D4g: listByConfig → 返回该 config 的活终端', async () => {
    ...
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t1', makeStartParams({ workspaceId: 'wsA', configId: 'c1' }));
    await mgr.start('t2', makeStartParams({ workspaceId: 'wsA', configId: 'c1' }));
    await mgr.start('t3', makeStartParams({ workspaceId: 'wsA', configId: 'c2' }));
    const c1Terms = mgr.listByConfig('c1');
    assert.equal(c1Terms.length, 2);
    assert.ok(c1Terms.every(t => t.configId === 'c1'));
    await mgr.stopAll();
});

test('D4h: status 返回 startedConfigName/kind', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t1', makeStartParams({ startedConfigName: 'my-cfg', kind: 'derived' }));
    const s = mgr.status('t1');
    assert.equal(s.startedConfigName, 'my-cfg');
    assert.equal(s.kind, 'derived');
    await mgr.stop('t1');
});

// ════════════════════════════════════════════════════════════
// D5 PTY onData → WS 广播（mock pty + 假 ws 客户端，keyed by terminalId）
// ════════════════════════════════════════════════════════════
test('D5a: PTY onData → 广播到所有 attached WS 客户端', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t5', makeStartParams());
    ...
    assert.equal(mgr.attachWs('t5', fakeWs), true);
    // 模拟 PTY 输出
...
test('D5e: 终端不存在时 attachWs → false', () => {
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: makeMockPty() });
    assert.equal(mgr.attachWs('nonexistent', { readyState: 1, OPEN: 1, on() {} }), false);
});

test('D5c: WS close → 移除客户端，PTY 保持（可重连/重入）', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t7', makeStartParams());
    const fakeWs = { readyState: 1, OPEN: 1, send() {}, on(event, cb) { this['_' + event] = cb; }, close() {} };
    mgr.attachWs('t7', fakeWs);
    fakeWs._close();
    // PTY 终端应仍在（可重入）
    assert.ok(mgr.status('t7'), 'PTY 终端应保持（可重连）');
    await mgr.stop('t7');
});
...
test('D6a: binaryPath 为空 → 启动拒绝', async () => {
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: makeMockPty() });
    await assert.rejects(() => mgr.start('t8', makeStartParams({ binaryPath: '' })), /二进制未找到|binary/i);
});

test('D6b: cwd 缺失 → 启动拒绝', async () => {
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: makeMockPty() });
    await assert.rejects(() => mgr.start('t9', makeStartParams({ cwd: '' })), /目录无效/i);
});

// ════════════════════════════════════════════════════════════
// D7 cleanup
// ════════════════════════════════════════════════════════════
test('D7a: stopAll → 停所有终端', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('t1', makeStartParams());
    await mgr.start('t2', makeStartParams());
    await mgr.stopAll();
    assert.equal(mgr.status('t1'), null);
    assert.equal(mgr.status('t2'), null);
});

// ════════════════════════════════════════════════════════════
// management API 终端路由（用真实 server + sessionPty mock 注入）
// 注：startManagementServer 接 opts.sessionPty 透传给 ClaudeSessionManager。
// ════════════════════════════════════════════════════════════
let mgmtSeq = 0;
async function startMgmt(label, opts = {}) {
    ...
    const handle = await startManagementServer({
        ...
        sessionPty: opts.sessionPty || makeMockPty(),
        ...opts,
    });
    return { handle, home, port };
}

// 创建 workspace + normal config 的辅助
async function createWsAndConfig(port, label, content = null) {
    ...
    const cfgContent = content || JSON.stringify({
    ...
    const cc = await (await fetch(`http://127.0.0.1:${port}/api/workspaces/${cr.workspace.id}/configs`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: 'cfg', content: cfgContent, mode: 'direct' }),
    })).json();
    return { proj, wsId: cr.workspace.id, cfgId: cc.config.id, cfg: cc.config };
}

test('D5route: POST /api/workspaces/:id/terminals workspace 不存在 → 404', async () => {
    const { handle, port, home } = await startMgmt('route404');
    try {
        const r = await fetch(`http://127.0.0.1:${port}/api/workspaces/ws_nonexist/terminals`, { method: 'POST' });
...
test('D5route: GET /api/workspaces/:id/terminals 无终端 → 200 + 空', async () => {
    const { handle, port, home } = await startMgmt('routeList');
    const { proj, wsId } = await createWsAndConfig(port, 'routeList');
    try {
        const r = await fetch(`http://127.0.0.1:${port}/api/workspaces/${wsId}/terminals`);
        assert.equal(r.status, 200);
        const data = await r.json();
        assert.equal(data.terminals.length, 0);
    } finally {
        await handle.stop();
        rmSync(home, { recursive: true, force: true });
...
test('GET /terminal/:tid → 终端页 HTML（含 /api/terminals/，无 claude-session）', async () => {
    const { handle, port, home } = await startMgmt('term');
    const proj = newTmpDir('term-proj');
    try {
        // 直接造一个终端：用 ClaudeSessionManager mock pty start（通过路由无法直接造，需先建 ws+config+activate）
        // 这里测 HTML 渲染：buildTerminalHtml 不依赖终端真实存在，GET /terminal/:tid 返回 HTML 即可
        const r = await fetch(`http://127.0.0.1:${port}/terminal/t_fake1`);
        assert.equal(r.status, 200);
        const html = await r.text();
        assert.ok(html.includes('xterm'));
        assert.ok(html.includes('/api/terminals/'), 'WS URL 应指向 /api/terminals/:tid/ws');
        assert.ok(!html.includes('claude-session'), '不应残留旧 claude-session 路径');
        assert.ok(!html.includes('method: \'POST\''), '不应有 POST-start（终端已存在，直接连 WS）');
    } finally {
        await handle.stop();
        rmSync(home, { recursive: true, force: true });
        rmSync(proj, { recursive: true, force: true });
...
// ── XSS-1: buildTerminalHtml terminalId 未转义 → JS 字符串注入 ──
test('REVIEW-XSS-1: buildTerminalHtml terminalId 含单引号 → 不应注入可执行 JS', async () => {
    ...
    const html = buildTerminalHtml({ terminalId: malicious, apiBase: '' });
    // 期望：terminalId 不被直接模板插值进 JS 上下文。
    // 修复后 tid 通过 JSON.stringify 嵌入（"';alert(1);//"），URL 通过 encodeURIComponent 拼接。
    // 关键标志：wsUrl 构造应通过 encodeURIComponent(tid)，而非直接插值。
    const wsUrlLine = html.split('\n').find(l => l.includes('wsUrl') && l.includes('encodeURIComponent'));
    assert.ok(wsUrlLine, 'wsUrl 应通过 encodeURIComponent(tid) 构造，而非直接插值');
});

// ── XSS-2: buildTerminalHtml terminalId 含 HTML → 不应产生可执行 HTML 元素 ──
test('REVIEW-XSS-2: buildTerminalHtml terminalId 含 < > → HTML 上下文应转义', async () => {
    const { buildTerminalHtml } = await import(pathToFileURL(resolve(__dirname, '..', '..', 'standalone', 'web', 'workspaces-html.js')).href);
...
test('REVIEW-RACE-1: PTY 自然退出后 ws message 不应写死 PTY', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('rev1', makeStartParams());
    const fakeWs = { readyState: 1, OPEN: 1, send() {}, on(event, cb) { this['_' + event] = cb; }, close() {} };
...
test('REVIEW-RACE-2: onData 广播一个 ws.send 抛错不影响其他客户端', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('rev2', makeStartParams());
...
test('REVIEW-RACE-2b: PTY onExit ws.send 抛错 → ws.close 仍执行', async () => {
    const mockPty = makeMockPty();
    const mgr = new ClaudeSessionManager({ log: () => {}, pty: mockPty });
    await mgr.start('rev2b', makeStartParams());
...
// ── RACE-3: handleUpgrade status 检查后 attachWs 前终端被 stop → ws 应被关闭 ──
test('REVIEW-RACE-3: WS upgrade 终端不存在 → ws 应被 close 1008（非孤儿）', async () => {
    // 集成测试：真实 managementServer，连真实 WS 客户端。
    // 终端不存在 → upgrade 应拒绝 + close ws（code 1008）。
    const { handle, port, home } = await startMgmt('rev3');
    try {
        // 直接连不存在的终端 WS → 应被拒
        const wsUrl = `ws://127.0.0.1:${port}/api/terminals/t_nonexist/ws`;
        ...
        assert.ok(closed.msgs.some(m => m.includes('终端') || m.includes('会话')), '应收到错误消息');
    } finally {
