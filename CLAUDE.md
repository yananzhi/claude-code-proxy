...
- **全量**（一条命令跑完所有 `node --test` 套件，约 65s）：
  ...
  现状 428 tests / 426 pass / 0 fail / 2 skip（POSIX 专属在 Windows 跳过）。
...
- **按目录跑**（改某块时针对性）：
  ...
  - `node --test test/standalone/` — 独立后端入口骨架 + workspace 管理（config 初始化 / spawn 守护 / 心跳 re-spawn / 生命周期 / workspace CRUD / management API，43 用例，起真 server.js 子进程）
