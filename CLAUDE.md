...
- **全量**（一条命令跑完所有 `node --test` 套件，约 65s）：
  ...
  现状 534 tests / 532 pass / 0 fail / 2 skip（POSIX 专属在 Windows 跳过）。
