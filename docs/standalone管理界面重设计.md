...
- 「激活」动作（`activateConfig`，写 settings.json + 注入 upstream）在新设计里：
  - standalone 管理页：**去掉激活按钮**（或保留为"设为默认起终端配置"的弱标记，不再写 settings.json）。
  - VS Code 扩展侧：保留（VS Code 里直接开终端仍需 active 配置）。
- `activateConfig` 函数本身不删（VS Code 侧仍用），但 standalone 路由不再调用它写 settings.json；起终端时按需注入 upstream。

> ⚠ 待确认：是否完全去掉 standalone 页的「激活」概念，还是保留一个"默认配置"标记用于「+ 新建终端」下拉的默认高亮。见第 6 节待定项。
...
| 「+ 新建终端」 | 单按钮，基于 active 配置 | **下拉 ▾**，选基于哪个配置新建 |
...
```
  [+ 新建终端 ▾]
      基于 [静态] glm-5.2 (直连)
      基于 [静态] kimi-k2 (代理)      ← active/默认高亮
      基于 [别名] #3  ccp-main-3
      基于 [别名] #4  ccp-sonnet-4
```
...
- 「+ 新建终端」单按钮 → 下拉 ▾，列出所有可选配置（静态 + 别名）。
- 「+ 新建配置」→ 「+ 新建静态配置」；"+ 派生"按钮 → "新建别名配置"。
- 配置编辑页文案：「派生节点」→「别名配置」。
...
- 「激活」路由：保留（VS Code 侧用），但 standalone 页面 UI 去掉激活按钮（或改为"默认配置"弱标记）——见第 6 节待定。
...
1. **「激活」概念 → 保留弱标记，不写 settings.json。**
   - `LocalActiveStateStore` 语义从"激活写文件"降级为"默认配置标记"，仅影响「+ 新建终端」下拉的默认高亮项（多配置时直接点「+ 新建终端」用默认那个，不用每次下拉选）。
...
2. **起终端入口 → 两者共存。**
   - 配置行内保留「新建终端」按钮（点某个具体配置直接起终端，意图明确、一步到位）；**静态配置行也加该按钮**（现在只有别名配置行有），统一。
   - 终端组的「+ 新建终端 ▾」下拉作为"总入口"（不知道选哪个时在这里选）。
   - 两个入口调同一个起终端 API，参数来源不同：行内按钮自带 cfgId；下拉需选 cfgId。

3. **别名终端顶栏真实模型 → 实时查代理 `modelAliases`。**
   - 终端页打开时 + 别名映射变更后，查代理 `GET /api/model-alias` 拿当前 `ccp-<tier>-N` → 真实模型映射，显示在顶栏「ccp-main-3 → glm-5.2」。
...
- [ ] 主列表页每个 workspace 下只有「配置」「终端」两个一级标签。
- [ ] 所有终端（静态 + 别名）统一显示在「终端」组，行首标 `[静态]`/`[别名]`。
- [ ] 别名配置仍挂在父静态配置下（↳ 缩进），标签为「别名配置 #N」。
- [ ] 起任何终端都不再读/写 settings.json，env 现场拼。
- [ ] 静态终端可直连、可代理；别名终端强制代理。
- [ ] 「+ 新建终端」可选基于任一配置（静态/别名）起终端。
- [ ] 别名配置编辑页改映射即时生效（代理侧 `modelAliases` 更新），终端页顶栏反映当前映射。
- [ ] 全站文案无"派生/derived/Local LLM Configs"残留（用户可见处）。
- [ ] 现有测试套件（`test/standalone/`）适配后全绿；新增终端统一走 env 的覆盖测试。
